...
The PIC MCU is mostly an event driven MCU with very limited resources (64K program space, 4K RAM, 1K EEPROM) so the code is written to use minimal resources for a real time system with no real operation system (yeah, ouch!). The library consumes bout 8K code and 600 bytes of RAM.
